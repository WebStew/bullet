...
export default connect (
  ...
  _renderLabel = (scene: TabScene) => {
    ...
    if (typeof label === 'string') {
      return (
        <Animated.Text style={[styles ( this.props.theme ).label, { color }, labelStyle]}>
          {label}
        </Animated.Text>
      );
    }
  ...
  render() {
    ...
    const inputRange = [-1, ...routes.map((x: *, i: number) => i)];
    return (
      <Animated.View style={[styles ( this.props.theme ).tabBar, style]}>
        {routes.map((route: NavigationRoute, index: number) => {
          ...
          return (
            ...
            >
              <Animated.View
                style={[styles ( this.props.theme ).tab, { backgroundColor, justifyContent }]}
              >
